...
*Session 2026-04-21 (Builds 63-64) — **Passe typo globale** (Build 63) : Space Grotesk (UI) + JetBrains Mono (build stamp) appliqués sur balance, build stamp, boutons bet, boutons lancer, popup gain, labels multiplicateurs cases. Labels cases w800 → w700 pour matcher spec §3. Décalage §7 "Typo globale" résolu. **Bouton (?) onboarding + alignement HUD top 40px** (Build 64) : nouveau widget `_HelpButton` (40×40, mêmes tokens que burger, `Icons.help_outline`) en `top:16 right:62` → relance le tour au step 1/4. Balance passe à `height:40` fixe, HUD top aligné (balance + ? + ⚙ même ligne). Tâche Notion cleanup `PlinkoTitle` dead code créée en Backlog pour visibilité.*

*Session 2026-04-21 (Builds 65-71) — **Passe wording + refonte layout onboarding**. Landing : punchline « Lâche. Prie. Encaisse. » + sous-titre supprimé (Build 65). Callouts onboarding minimalistes : retrait bouton « Passer » top-right, progress bar top, step pill, dots, et body/sous-titre (Builds 66-68). Plus que le titre + les CTAs inline (Passer ghost link à gauche de Suivant, step 4 : uniquement Terminer). Titres finaux portent toute l'explication : step 1 « La bille tombe, rebondit et atterrit dans une case. », step 2 « Les bords paient gros. Le centre, bien moins. », step 3 « Sélectionne la valeur de chaque bille. », step 4 « Tu peux choisir entre 1 et 10 billes par lancer. ». **Refonte targets** (Builds 67/71) : step 1 cible le plateau entier (bille + pyramide + cases, plus le wordmark) ; step 2 cible une bande resserrée sur la rangée multiplicateurs seule (nouveau `_multipliersKey`). Bascule des targets de `Padding` en % vers `Positioned(left/right/top/bottom)` explicite après débordement horizontal observé sur iOS Safari.*
