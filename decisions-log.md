...
| 2026-04-21 | Design | **Build 65 — passe wording landing + onboarding** : punchline landing « Tombe. Rebondit. Gagne. » → « Lâche. Prie. Encaisse. » + sous-titre supprimé. Titres onboarding réécrits (enjeu > mécanique, zéro jargon) : « Le plateau » → « Valeur des cases », « Billes par lancer » → « Nombre de billes ». Spec Notion validée 2026-04-20 appliquée. |
| 2026-04-21 | Design | **Build 66 — callouts onboarding allégés** : retrait bouton « Passer » flottant top-right + progress bar cyan top (redondante avec dots + step pill). « Passer » devient ghost link inline à gauche de Suivant (`Row` end-aligned, gap 8px). Step 4 : uniquement « Terminer » (showSkip=false). UI tour = callout + spot ring uniquement. |
| 2026-04-21 | Design | **Build 67 — refonte targets onboarding** : step 1 cible désormais le plateau entier (bille + pyramide + cases) au lieu du wordmark. Step 2 cible une bande resserrée sur la rangée multiplicateurs seule (nouveau `_multipliersKey`). Brief issu du commentaire Notion 2026-04-20 ("la bille comprise dans le rectangle", "popup au-dessus du plateau picots"). |
| 2026-04-21 | Design | **Build 68-70 — callouts minimalistes (titre seul)** : retrait rangée top (step pill `1/4` + dots breadcrumb) puis retrait body/sous-titre. Titres réécrits pour porter toute l'explication : step 1 « La bille tombe, rebondit et atterrit dans une case. », step 2 « Les bords paient gros. Le centre, bien moins. », step 3 « Sélectionne la valeur de chaque bille. », step 4 « Tu peux choisir entre 1 et 10 billes par lancer. ». Raison : hauteur mobile contrainte, éviter de rogner sur le plateau. |
| 2026-04-21 | Dev | **Build 71 — positionnement explicite des targets onboarding (fix iOS)** : bascule des zones `_boardKey` et `_multipliersKey` de `Padding(EdgeInsets.only(top:%, bottom:%))` vers `Positioned(left:2%, right:2%, top:%, bottom:%)` dans un Stack. L'heuristique `Padding` débordait horizontalement sur iPhone Safari (rect décalé, spot à moitié hors champ malgré un calcul logique centré). `Positioned` explicite force le cadre dans le viewport quel que soit le renderer. |
